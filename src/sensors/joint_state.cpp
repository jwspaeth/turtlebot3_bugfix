...
: Sensors(nh, frame_id)
{
  pub_ = nh->create_publisher<sensor_msgs::msg::JointState>(topic_name, 
this->qos_);

  RCLCPP_INFO(nh_->get_logger(), "Succeeded to create joint state 
publisher");
}
...
https://social.msdn.microsoft.com/Forums/vstudio/en-US/e6e1d2a0-70e3-4a91-bc84-e634b5bbda98/what-is-this-memory-location?forum=vclanguage
  // TODO: dxl_sdk_wrapper->get_data_from_device returns memory access 
exception, which is not handled and therefore odometry value is calculated 
  // with exception value (0xFDFDFDFD), which cause huge position change.
  // Visualisation of this issue can be seen here: 
https://github.com/ros-planning/navigation2/issues/3117
  // This is just a temporary workaround to make it Manwork. Exception 
should be corectly handled

  static constexpr auto NoMansLand = 
static_cast<std::int32_t>(0xFDFDFDFD);
